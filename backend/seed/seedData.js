...
const seedDatabase = async () => {
  ...
  // Generate 70 incidents spread over the last 12 months
  const now = new Date();
  ...
  for (let i = 0; i < 70; i++) {
    ...
    // Weighted status distribution: more closed, some open, some pending
    ...
    const incident = new Incident({
      ...
      reportedByPhone,
      peopleInvolved: [getRandom(names), getRandom(names)].filter((v, i, a) => a.indexOf(v) === i),
      description,
      actionTaken,
  ...
  const notifications = [
    ...
    },
    {
      ...
      message: `Incident ${createdIncidents[2].incidentId} has been marked as Closed.`,
      type: 'status_change',
      incidentId: createdIncidents[2]._id,
      forRoles: ['admin', 'dept_head'],
      isRead: false,
    },
    {
      title: 'New Medication Error Reported',
      message: 'A Medium severity Medication Error has been reported in Pharmacy.',
      type: 'new_incident',
      incidentId: createdIncidents[3]._id,
      forRoles: ['admin', 'dept_head'],
      isRead: true,
    },
  ];
  ...
  console.log('  Dept Head   depthead@hospital.com  dept123');
  console.log('  Staff       staff@hospital.com     staff123');
  console.log('  Viewer      viewer@hospital.com    viewer123');
  console.log('═══════════════════════════════════════════════\n');
